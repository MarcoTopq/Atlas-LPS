...
export default function NaskahDinasOutboxPage() {
  ...
  return (
    <div className="flex flex-col min-h-dvh bg-[#F8FAFC] pb-24 relative">
      <AppBar title="Outbox Naskah Dinas" showBack />
      ...
      <div className="px-5 mt-4 space-y-6">
        ...
        <div className="flex flex-col gap-4">
          {filteredTasks.map((task, idx) => (
            <Link 
              href={`/naskah-dinas/outbox/${task.id}`} 
              key={idx}
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <span className={cn(
                  ...
                  task.status === "Terkirim" ? "bg-[#1E9E6A]" : "bg-amber-500"
                )}>
                  {task.status}
                </span>
                <span className="text-[11px] font-semibold text-muted">{task.tgl}</span>
              </div>
              ...
              <div className="pt-3 border-t border-line text-[12px]">
                <span className="text-muted text-[11px]">Penerima: <strong className="text-ink">{task.penerima}</strong></span>
              </div>
            </Link>
          ))}
        </div>
