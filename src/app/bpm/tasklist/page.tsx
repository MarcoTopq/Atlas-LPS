...
import { MOCK_BPM_TASKS } from "@/lib/mock/bpm";
import { Search, SlidersHorizontal } from "lucide-react";
import Link from "next/link";
import { useState } from "react";

export default function BpmTasklistPage() {
  const [searchTerm, setSearchTerm] = useState("");

  const filteredTasks = MOCK_BPM_TASKS.filter(t => 
      ...
      <div className="px-5 mt-4 space-y-6">
        ...
        <div className="flex flex-col gap-4">
          {filteredTasks.map((task, idx) => (
            <Link 
              href={`/bpm/tasklist/${task.id}`} 
              key={idx}
              className="bg-white rounded-[24px] p-5 shadow-[0_4px_20px_rgba(0,0,0,0.02)] hover:shadow-[0_8px_24px_rgba(0,0,0,0.05)] border border-slate-50 transition-all flex flex-col"
            >
              <div className="mb-4">
              ...
              <div className="grid grid-cols-2 gap-y-4 gap-x-2">
                ...
                <div className="flex flex-col">
                  <span className="text-[11px] font-medium text-muted mb-0.5">Direview</span>
                  <span className="text-[13px] font-semibold text-ink">{task.direview}</span>
                </div>
                <div className="flex flex-col col-span-2">
                  <span className="text-[11px] font-medium text-muted mb-0.5">Approver Selanjutnya</span>
                  <span className="text-[13px] font-semibold text-ink">{task.approver}</span>
                </div>
              </div>
            </Link>
          ))}
